...
def sort(data, args, idx_train, idx_val, device):
    ...
    save_dir = os.path.join(current_dir, f"sorted_samples/{args.trigger}/{args.dataset}")
...
def spear_select(data, args, idx_train, idx_val, device):
    current_dir = os.getcwd()
    save_dir = os.path.join(current_dir,  f"sorted_samples/{args.trigger}/{args.dataset}")
    file_path = os.path.join(save_dir, "sorted_samples.pt")
...
def cluster_degree_selection(args,data,idx_train,idx_val,idx_clean_test,unlabeled_idx,train_edge_index,size,device):
    ...
    if os.path.exists(file_path):
        print(f"Loading selected samples from: {file_path}")
        idx_attach = torch.load(file_path, map_location=device)
    else:
        print("sorted_samples.pt not found")
        ...
        torch.save( idx_attach, file_path)
